...
```bash
terraform/scripts/run-external-iam-validation.sh \
  --tfvars terraform/external-iam-validation.tfvars \
  --backend-config terraform/backend.external-iam-validation.hcl
```
